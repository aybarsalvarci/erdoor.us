...
<div x-data="mediaPicker('{{ $value }}', '{{ $name }}', {{ $multiple ? 'true' : 'false' }}, '{{ $returnType }}')"
     @open-tinymce-media.window="if($event.detail.target === '{{ $name }}') openModal(true)"
     class="{{ $hideTrigger ? '' : 'mb-3' }}"
     x-cloak>

    <!-- TETİKLEYİCİ ALAN (Sayfa İçi Görünüm) -->
    @if(!$hideTrigger)
                ...
                <div class="p-3 rounded border border-secondary bg-transparent">
                    ...
                    <div class="gallery-preview-container">
                        ...
                        <template x-if="selectedItems.length === 0">
                            <div class="gallery-preview-item d-flex align-items-center justify-content-center"
                                 style="border: 1px dashed rgba(150,150,150,0.5);">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="button" x-show="selectedItems.length > 0" @click="selectedItems = []"
    ...
    <template x-if="isOpen">
        <div class="media-modal-backdrop">
            <!-- Modal Kapsayıcı -->
            <div class="card shadow-lg w-100 mx-3 border-secondary"
                 style="background-color: var(--card-bg, #0c1427); max-width: 1000px; height: 85vh;">

                <!-- Modal Başlık -->
                <div
                    class="card-header d-flex justify-content-between align-items-center border-bottom border-secondary"
                    style="background-color: rgba(0,0,0,0.15);">
                    <h5 class="card-title mb-0 text-primary fw-bold">Ortam Kütüphanesi <span
                            x-show="isMultiple && !isTinyMceMode" class="text-muted small ms-2">(Çoklu Seçim)</span>
                    </h5>
                ...
                <div class="px-3 pt-2 border-bottom border-secondary" style="background-color: rgba(0,0,0,0.1);">
                    <ul class="nav nav-tabs border-0" role="tablist">
                        <li class="nav-item">
                            <button class="nav-link text-body" :class="{ 'active fw-bold': activeTab === 'library' }"
                                    @click="activeTab = 'library'" type="button" style="background-color: transparent;">
                                Ortam Kütüphanesi
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link text-body" :class="{ 'active fw-bold': activeTab === 'upload' }"
                                    @click="activeTab = 'upload'" type="button" style="background-color: transparent;">
                                Bilgisayardan Yükle
                            </button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link text-body" :class="{ 'active fw-bold': activeTab === 'url' }"
                                    @click="activeTab = 'url'" type="button" style="background-color: transparent;">
                                URL'den Ekle
                            </button>
                        </li>
                    </ul>
                ...
                <div class="card-body d-flex p-0 overflow-hidden" style="min-height: 0;">
                    ...
                    <div class="flex-grow-1 position-relative border-end border-secondary"
                        ...
                        <!-- 1. KÜTÜPHANE SEKMESİ (d-flex/d-none sınıfı ile kontrol ediliyor) -->
                        <div :class="activeTab === 'library' ? 'd-flex' : 'd-none'"
                             class="h-100 flex-column p-3 overflow-y-auto">
                                ...
                                <template x-for="item in items" :key="item.id">
                                    <div class="col">
                                        <div @click="toggleSelection(item)"
                                             :class="checkIfSelected(item) ? 'border-primary border-2 shadow' : 'border-secondary border'"
                                             class="card h-100 cursor-pointer position-relative overflow-hidden select-none bg-transparent">
                                            <img :src="item.resolved_url"
                                                 class="card-img-top object-fit-cover w-100 h-100"
                                                 style="aspect-ratio: 1/1;" :alt="item.alt_text">
                                            ...
                                            <div x-show="checkIfSelected(item)" class="selected-overlay">
                                                <div
                                                    ...
                                                    <i data-feather="check" class="text-white"
                        ...
                        <!-- 2. BİLGİSAYARDAN YÜKLE SEKMESİ (d-flex/d-none sınıfı ile kontrol ediliyor) -->
                        <div :class="activeTab === 'upload' ? 'd-flex' : 'd-none'"
                             class="h-100 flex-column align-items-center justify-content-center p-4">
                            <div
                                class="border border-2 border-dashed border-secondary rounded p-5 text-center w-75 bg-transparent"
                                @dragover.prevent @drop.prevent="handleDrop($event)">
                                <input type="file" @change="uploadFile($event.target.files[0])" class="d-none"
                                       id="file_picker_{{ $name }}"
                                       accept="image/png, image/jpeg, image/webp, image/gif">
                                <i class="d-block mb-3 text-muted mx-auto" data-feather="upload-cloud"
                                ...
                                <label for="file_picker_{{ $name }}"
                                       class="cursor-pointer fw-bold btn btn-primary btn-sm mb-4">Bilgisayardan
                                    Seç</label>
                                <input type="text" x-model="uploadAltText"
                                       placeholder="Yüklemeden Önce SEO Alt Text (Opsiyonel)"
                                       class="form-control form-control-sm text-body bg-transparent border-secondary"/>
                            </div>
                        </div>

                        <!-- 3. URL'den EKLE SEKMESİ (d-flex/d-none sınıfı ile kontrol ediliyor) -->
                        <div :class="activeTab === 'url' ? 'd-flex' : 'd-none'"
                             class="h-100 flex-column align-items-center justify-content-center p-4">
                            <div
                                class="border border-2 border-dashed border-secondary rounded p-5 text-center w-75 bg-transparent">
                                <i class="d-block mb-3 text-muted mx-auto" data-feather="link"
                                   style="width: 48px; height: 48px;"></i>
                                <h6 class="text-body fw-bold mb-4">Dış Kaynaktan (URL) Görsel Ekle</h6>
                                <div class="input-group mb-3">
                                    <input type="url" x-model="externalUrl" placeholder="https://ornek.com/gorsel.jpg"
                                           class="form-control text-body bg-transparent border-secondary">
                                    <button type="button" @click="uploadFromUrl()" class="btn btn-primary px-4"
                                            :disabled="isUploading">Ekle
                                    </button>
                                </div>
                                <input type="text" x-model="uploadAltText" placeholder="SEO Alt Text (Opsiyonel)"
                                       class="form-control form-control-sm text-body bg-transparent border-secondary"/>
                            </div>
                        </div>
                    ...
                    <div class="p-3 d-flex flex-column justify-content-between shrink-0 bg-transparent"
                         style="width: 300px; flex-shrink: 0;">

                        <!-- Tekli Seçim Detayı ve Auto-Save Alt Text Alanı -->
                        <div x-show="!isMultipleActive() && activeItems.length === 1" class="overflow-y-auto pe-1">
                            ...
                            <div
                                class="border border-secondary rounded p-2 mb-3 d-flex align-items-center justify-content-center"
                                style="background-color: rgba(0,0,0,0.15); height: 160px;">
                                <img :src="activeItems[0]?.resolved_url"
                                     class="img-fluid rounded object-fit-contain w-100 h-100" alt="Önizleme">
                            </div>

                            <p class="text-sm text-body fw-bold text-truncate mb-1"
                               x-text="activeItems[0]?.file_name"></p>
                            <p class="text-xs text-muted mb-4" x-text="activeItems[0]?.date"></p>

                            <!-- Otomatik Kayıt (Auto-Save) Alt Text Kutusu -->
                            <div class="mb-2">
                                <label class="form-label text-xs text-muted d-flex justify-content-between mb-1">
                                    Alternatif Metin
                                    <span class="text-success" x-show="altSaved"
                                          style="font-size: 10px;">Kaydedildi!</span>
                                </label>
                                <textarea class="form-control text-body bg-transparent border-secondary" rows="3"
                                          style="font-size: 13px;" x-model="activeItems[0].alt_text"
                                          @change="updateAltText(activeItems[0])"
                                          placeholder="Görseli betimleyin..."></textarea>
                                <small class="text-muted d-block mt-1" style="font-size: 11px;">Odağı kaybettiğinde
                                    otomatik kaydedilir.</small>
                            </div>
                        </div>
                        ...
                        <div x-show="isMultipleActive() && activeItems.length > 0" class="text-center mt-5">
                            <div
                                class="bg-primary rounded-circle d-inline-flex align-items-center justify-content-center mb-3 shadow"
                        ...
                        <!-- Onay Butonu (En Altta Sabit) -->
                        <div class="mt-3 pt-3 border-top border-secondary">
                            <button type="button" @click="confirmSelection()" :disabled="activeItems.length === 0"
...
<script>
    document.addEventListener('alpine:init', () => {
        if (!Alpine.data('mediaPicker')) {
            Alpine.data('mediaPicker', (initialValue, componentName, isMultipleConfig, returnTypeConfig) => ({
                isOpen: false,
                isTinyMceMode: false,
                ...
                init() {
                    ...
                    if (isNaN(initialValue) && (initialValue.startsWith('http') || initialValue.startsWith('/'))) {
                        this.selectedItems = [this.formatMedia({
                            id: initialValue,
                            path: initialValue,
                            type: 'external'
                        })];
                    ...
                    if (!isNaN(initialValue)) {
                        this.preloadByIds([initialValue]);
                    } else if (initialValue.startsWith('[') || initialValue.includes(',')) {
                        const ids = initialValue.replace(/[\[\]]/g, '').split(',').map(Number).filter(Boolean);
                        if (ids.length > 0) this.preloadByIds(ids);
                    }
                },
                ...
                formatMedia(media) {
                    ...
                    const dateStr = !isNaN(dateObj) ? dateObj.toLocaleDateString('tr-TR', {
                        day: '2-digit',
                        month: 'short',
                        year: 'numeric'
                    }) : '';
                ...
                preloadByIds(ids) {
                    fetch(`{{route('admin.media.fetch')}}?selected_ids=${ids.join(',')}`, {
                        headers: {'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest'}
                    })
                        .then(res => res.json())
                        .then(data => {
                            const list = Array.isArray(data) ? data : [data];
                            this.selectedItems = list.map(i => this.formatMedia(i));
                            setTimeout(() => {
                ...
                uploadFile(file) {
                    ...
                    fetch('{{route('admin.media.store.file')}}', {
                        ...
                        .finally(() => {
                            document.getElementById('file_picker_{{ $name }}').value = '';
                        });
                },
