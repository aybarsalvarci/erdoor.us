...
@section('content')
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0">Bülteni Düzenle</h4>
        </div>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-4">Bülten Bilgilerini Güncelleyiniz</h6>
                    ...
                    <!-- Bülten Düzenleme Formu -->
                    <form action="{{ route('admin.newsletter.update', $newsletter->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea
                                ...
                                name="body"
                                rows="15"
                                placeholder="Bülten içeriğini buraya yazınız...">{{ old('body', $newsletter->body) }}</textarea>
                            @error('body')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('button_text', $newsletter->button_text) }}">
                                @error('button_text')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="url"
                                       ...
                                       value="{{ old('button_link', $newsletter->button_link) }}">
                                @error('button_link')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="datetime-local"
                                       ...
                                       id="send_at"
                                       name="send_at"
                                       value="{{ old('send_at', $newsletter->send_at ? \Carbon\Carbon::parse($newsletter->send_at)->format('Y-m-d\TH:i') : '') }}">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                                    <option value="published" {{ old('status', $newsletter->status) == 'published' ? 'selected' : '' }}>Aktif (Yayına Al)</option>
                                    <option value="draft" {{ old('status', $newsletter->status) == 'draft' ? 'selected' : '' }}>Pasif (Taslak)</option>
                                </select>
                                @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="d-flex gap-2 mt-2">
                            ...
                            <a href="{{ route('admin.newsletter.index') }}" class="btn btn-outline-secondary d-flex align-items-center">
                                <i data-lucide="x" class="icon-sm me-2"></i> İptal
                            </a>
                        </div>
                    </form>
